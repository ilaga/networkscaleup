...
There is also additional functionality available to fit these
models using `cmdstan`.
...
We have created a version of this package which can use `cmdstanR`.
...
This requires an installation of `cmdstanR`. See [here](https://mc-stan.org/cmdstanr/)
for details.
...
We can then fit the (true) model to this data and evaluate the
fit of this model. Several diagnostics are provided, 
including rootograms and residual computation.
...
We see that the hanging rootogram indicates good fit, as would be expected.
More flexible models and additional model checking diagnostics
are also available.
