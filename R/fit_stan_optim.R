...
fit_stan_optim = function(ard,
  ...
  if(is.null(x_cov_global) & is.null(x_cov_local)){ # No cov
    stan_data = list(
      y = ard,
      n_i = nrow(ard),
      n_k = ncol(ard)
  ...
  }else if(is.null(x_cov_global)){ # Only subpop cov
    stan_data = list(
      y = ard,
      n_i = nrow(ard),
      n_k = ncol(ard),
  ...
  }else if(is.null(x_cov_local)){ # Only global cov
    stan_data = list(
      y = ard,
      n_i = nrow(ard),
      n_k = ncol(ard),
